...
/** 排序按钮组 + 模糊查询框：玩家端「参与共建」与后台「共建审核」共用同一套控件，
    ...
    children 是这一组控件下面还要跟着的、同样固定在列表上方的东西（玩家端的
    「＋ 投稿随机任务」入口；后台审核没有，就不传）。它必须放进本组件这个根容器里，
    不能作为 .contribute-sidebar 的兄弟节点平铺——右边缘对齐依赖根容器上那份与
    .contribute-list 一模一样的滚动条槽声明，详见 styles.css 里
    .contribute-list-controls 的注释。 */
export function ContributionListControls({ kind, sort, onSort, search, onSearch, searchPlaceholder, children }: {
  ...
  const ratio = ratioCycle(kind);
  return (
    <div className="contribute-list-controls">
      <div className="contribute-sort-row">
        <button
          type="button"
          className="small"
