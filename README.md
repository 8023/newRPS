...
面板按「核心趋势」「设备与来源」「游戏与惩罚」「互动与行为」「转化与明细」分组；趋势、双轴、堆叠、组合和圆环图统一在图表下方显示静态图例，精确数值通过悬停气泡查看。转化漏斗的页面浏览、来源、省份和最近会话仍可在「明细」页签中切换表格；长类目标签会在坐标轴上截断，悬停时保留完整名称，窄屏会自动调整卡片和筛选条布局。
...
- **v3.1.4：低风险精简与主宠图谱稳定性修复**：删除确认无调用的旧前端样式、展示/缓存辅助函数和无价值 Go 包装，移除未被项目直接导入的 `d3-shape` 顶层依赖；主宠关系图谱的坐标夹紧改为写回实际物理模拟节点，避免拖拽或力模拟把节点带出可见区域。保留 `HumanSeat`、LayerChart 间接依赖、Protobuf、数据库迁移和玩家玩法行为不变。
- **v3.1.3：后台分析性能与稳定性修复**：分析快照预编码 7/30/90 天应答，管理员预加载分析组件，图表滚动到视口附近才挂载，并统一图表高度、坐标轴留白、长类目省略与横向多系列图例；同时收紧安全日志/SQLite 文件权限，串行化玩家快照落盘，补齐白给自救板、名字争夺战和管理员认领密钥的持久化审计，限制匿名 session 请求体，修复共建编辑后的旧缓存、黑白棋极限连胜结算、主宠图谱拖拽、离房残留房态、惩罚证明上传竞态和 Svelte 动态样式单位问题。分析口径、协议和玩家端玩法不变。
- **v3.1.2：后台数据分析面板体验优化**：分析图表统一使用绘图区下方静态图例与悬停数值提示，移除重复的图表/表格切换和废弃系列表组件；新增五个主题分组标题，优化明暗主题配色、KPI/图表卡片、留存热力图和窄屏布局；长类目轴标签自动截断但气泡保留完整名称，并补齐猜硬币的游戏分析名称映射。分析口径、RPC、数据库和玩家端玩法均不变。
- **v3.1.1：React → Svelte 5 完成迁移**：登录、大厅、房间、8 款游戏、聊天、个人资料、社交、共建、后台与数据分析全部切换到按功能目录组织的 Svelte 组件；Go 后端、Protobuf 通信、主题与 `bin/dist/` 部署产物路径保持不变。前端构建改用 `svelte-check`，Vite 增加懒加载场景边界检查与图片处理分包，登出时会清理当前设备的 Web Push 订阅。
