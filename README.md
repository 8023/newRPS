...
| `contribution:list` / `get` / `saveDraft` / `submit` / `withdraw` / `requestUnpublish` | 玩家共建投稿的查询、草稿、送审、撤回与下架申请 RPC；动态载荷仍封装在 Protobuf `Struct` 中 |
| `contribution:votePreview` / `vote` | 正式投稿任务的评价预览与赞踩；未投票前只返回资格和任务标识，成功投票后才返回点赞率、票数与贡献者 |
| `admin:action` 的 `contribution*` / `genders*` | 共建审核、下架、评价覆盖与性别/阵营管理；仅管理员可调用 |
...
- 服务端留存：共建图片一旦上传成功并登记到数据库，就作为监管审计材料永久保留；即使从未绑定投稿、后来换图、修订或下架，业务服务也不会自动删除文件或记录。30 天只是浏览器/共享缓存寿命，不是服务器保留期；如需清理，只能由站外专用审计清理程序另行执行。
...
| `titles.json` | 称号段 |
| `punishments.json` | 惩罚配置：`tags`（标签，含房名词库/背景图库）+ 全局 `orderStep`/`maxDifficultyOvershoot`/`minSeriesSteps`（系列投稿最低步数，默认 10）/`maxSeriesSteps`（管理员可配置的系列投稿最高步数，默认 20）。最低/最高值均不得超过 1000，最高值不得小于最低值，后台保存越界配置会明确报错；1000 同时是后端拦截畸形 payload 的硬上限。任务池与系列任务详情已迁到 SQLite（`punishment_tasks` / `punishment_series`，schema v34 起系列表含 `target_faction_ids`），且新内容只能通过玩家投稿 + 管理员审批（`contribution:*` / `admin:action` 的 `contribution*` 系列）产生；管理员可在共建审核详情中修订原稿并立即发布，但后台不提供绕过投稿记录、直接操作正式任务池/系列表的入口。旧文件里的 `tasks`/`seriesTasks` 启动时一次性导入后不再读写 |
...
- **共建投稿**：任务池/系列任务不再走后台绕过投稿记录的直接增删改，一律保留「投稿版本 + 管理员审核」链路；管理员可在审核详情点「编辑」修订原稿，保存后立即发布，无需投稿者再次审核。已上线内容的玩家修订使用「修订草稿 / 修订审核中 / 修订被驳回」独立状态，复审期间旧版继续可玩。点赞只能投给具体任务（`task_group_id`：独立任务一份投稿一组，系列每一步一组），不能投给整个系列；系列点赞率为各个已有投票步骤点赞率的算术平均，尚无人评价的步骤不拖低均值。投票前服务端不下发点赞率、票数和贡献者，投票成功后才揭示。系列完成率在房间销毁时按「已走步数/总步数」百分比样本取均值。性别/阵营运行时权威在 SQLite（`genders.json` / `gender-factions.json` 只作空库种子）；后台删除性别后，引用它的玩家会重置为当前列表第一项并写回数据库。共建图片上传成功后永久留存，不随草稿、修订或下架自动清理。schema v33–v40，其中 v39 删除任务名称列，v40 为投稿聚合、随机池和历史事件补齐索引。详见 CHANGELOG「共建投稿」。
...
- **系列任务未覆盖阵营改为随机池替补**：某一步没有变体命中受罚者阵营时，从随机任务池抽一条顶替（难度按当前步序/总步数比例，标签三态由该系列自身词频推导并逐级放宽），进度照常推进。协议删除 `PunishmentRandomSettings.series_faction_fallback_text`（字段号 4 reserved），新增 `min_series_steps` 与 `PunishmentSeriesSummary.target_faction_ids`（schema v34）。建房面板不展示面向阵营；进战斗席由前端按 `target_faction_ids` 做知情确认。
- **后台惩罚内容改为审核制**：旧的「系列任务 / 任务池」直接编辑入口及保存 action 已删除；后台保留全局惩罚配置、性别与阵营、共建审核、批注、下架和评价覆盖。管理员可以在共建审核详情中编辑原稿并立即发布，但所有正式任务/系列仍统一经投稿版本事务发布并保留审计记录。
