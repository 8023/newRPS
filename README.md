...
| GET | `/ws` | WebSocket |
| GET | `/uploads/*` | 上传文件（证明图/头像/后台图）。要求 `Referer` 为本站页面（与 `ALLOWED_ORIGINS`/本机开发同一套判断），缺失或跨站一律 404，见下方说明 |
| GET | `/*` | 前端静态（`bin/dist/`） |
...
- 访问侧：文件名本身不可猜测（随机后缀），但这不是身份鉴权；`GET /uploads/*` 额外校验
  ...
  可以随意伪造 `Referer`，只作为纵深防御的一层，不改变"文件名泄露即可访问、且当前无过期
  /清理机制"这个前提，分享出去的证明图链接请自行注意。
  - ⚠️ 这层校验只在请求真正打到 Go 进程时才会执行——若响应被标成可共享缓存（`public`），
    ...
    这点残留风险，因此 `Cache-Control` 仍是 `public`，但把 `max-age` 从最初的 30 天收窄
    到 6 小时（多数房间的存活时长量级），缩小"链接泄露后经共享缓存仍可访问"的窗口；
...
- **系列任务进度改为房间级共享**：进度周期与房间同步——开房选定系列后全员共用一个步数计数器，谁挨罚都推进同一步（A 做完 5 条离开，B 从第 6 条继续）；换房间（即便开同一个系列）从头开始，房间销毁进度即释放。替代此前按玩家个人落盘、跨房间跨对手持续推进的模式（schema v32 删除废弃表 `player_punishment_series_progress`，后台「重置所有人进度」action 与按钮一并移除——房间共享进度无需人工重置，散场即清零）。上一条的阵营兜底文案行为不变，兜底步同样推进房间进度。
- **系列任务阵营覆盖放宽为兜底文案**：某一步的候选任务没覆盖全部阵营时系列照常保存、生效（此前整个系列会被判不可用：建房面板选不到、运行时不产出任务）——受罚者阵营在该步无命中时下发管理员可配的兜底文案（后台「惩罚任务 → 惩罚配置 → 系列任务兜底文案」，支持 `{loser}`/`{winner}`，默认「该任务不适用于你所在的阵营」），进度照常推进。协议 `PunishmentRandomSettings` 新增 `series_faction_fallback_text` 字段（存于 `punishments.json`），前后端需同步发布。
- **后台惩罚任务界面调整**：「保存任务池」「保存系列任务」按钮移至「惩罚配置 / 系列任务 / 任务池」子 Tab 行右侧（原底部保存行移除）；左侧列表（标签/任务/系列索引）高度约 +65%（约 7.5 条 → 12 条）；修复系列任务第二步及以后点「添加」不展开任务列表的问题（`<details>` 折叠态遮住了选择面板）。
- **无标签惩罚任务改为常驻随机池**：任务池里未勾选任何标签的任务不再被随机模式排除——它们无视建房时的标签选中/拒绝筛选，任何随机房间都可能抽到（此前无标签任务永远抽不到）。想专门留给系列任务用的任务，请把难度设为 -1（这是现在唯一的"不参与随机"标记）。阵营勾选逻辑不变：未勾选阵营的任务仍然抽不到。
