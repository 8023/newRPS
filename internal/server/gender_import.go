...
func (s *Server) importGendersFromJSONIfNeeded() error {
	...
	// gender_options 建了 (faction_id, label) 唯一约束，种子 JSON 一旦手误写出同阵营重名
	// 选项，原样导入会在 INSERT 时撞约束、整个事务回滚——表继续保持空，随后启动校验只会
	// 报出与真实原因无关的"至少需要一个性别选项"。这里按 (faction_id, 规范化 label) 去重
	// （保留 JSON 中先出现的那条），并把被丢弃 ID 映射到保留 ID，供导入成功后回填
	// players.gender_id，避免存量玩家的性别选择变成悬空引用。纯粹是防御性兜底，不代表
	// 线上真的出现过这种重名种子。
