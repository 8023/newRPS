...
func TestCandidateTasksForTags(t *testing.T) {
	tasks := []types.PunishmentTaskConfig{
		...
		{ID: "d", TagIDs: nil}, // 未勾选任何标签：任何过滤条件下都不应出现
	}
	...
	if want := []string{"a", "b"}; !sameStringSlice(ids(got), want) {
		t.Fatalf("include truth: got %v want %v", ids(got), want)
	}
	// S={truth,challenge} R={} → 只有 b
	got = candidateTasksForTags(tasks, []string{"truth", "challenge"}, nil)
	if want := []string{"b"}; !sameStringSlice(ids(got), want) {
	...
	// S={challenge} R={sing} → 空（c 有 sing，b 没有 challenge? b 有 challenge 且无 sing → b）
