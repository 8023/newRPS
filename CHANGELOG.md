...
本版本对**共建投稿系统架构**进行深度重构：从双层投稿信封模型（`contribution_submissions` 信封层 + 版本层）改为单一版本化表（`sub_tasks` / `series`）；性别投稿功能正式下线，性别与阵营管理移至后台独占；投稿编辑流程调整，编辑已通过内容**立即下线、需重新审批**后才生效；玩家表拆分为主表 + 子表模式，减少主表宽度。六次数据库迁移（v41–v46）清理冗余列与表、简化数据模型、优化索引，一次性迁移（v43）在失败时回滚确保无中间状态。
...
- **性别投稿下线**：玩家不再能通过「参与共建」投稿性别/阵营（原有界面入口已移除）。运行时性别与阵营的权威来源仍为 SQLite，但编辑只能通过后台「性别与阵营」卡片进行。旧的性别投稿记录在 v43 迁移时保留作为审计档案（新表有 `contributor_id` 等追溯字段），但不再有投稿者身份的新增渠道。
