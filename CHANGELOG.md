...
本版本深化棋类惩罚与白给体验，并重做惩罚体系三项规则：国际象棋与斗兽棋新增「每子惩罚」——吃子即触发惩罚、审批后对局继续（棋钟暂停、不计分不改白给值）；斗兽棋补上白给机制（轮到你走子时按白给值一半的概率直接跳过本手，主人也可强制宠物跳过）；排位赌分档位改为按游戏在后台配置，用于平衡各游戏耗时。系列任务进度改为房间级共享、某一步未覆盖阵营时改为下发兜底文案而非整系列作废；任务池未勾选标签的任务改为常驻随机池，难度 -1 成为「不参与随机」的唯一标记；后台惩罚任务界面同步调整。
...
- **唯一排除手段**：想专门留给系列任务用的任务，把难度设为 -1（这是现在唯一的「不参与随机」标记）；阵营勾选逻辑不变，未勾选阵营的任务仍然抽不到。
