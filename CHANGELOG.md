...
本版本上线**共建投稿系统**和**惩罚难度独立计数**，彻底重构玩家创作体验与惩罚系统。大厅新增「参与共建」，持久玩家可投稿性别、随机任务、系列任务，经后台审核后进入正式数据；管理员不再有绕过投稿记录、直接编辑任务池/系列表的入口，所有正式内容统一走投稿版本事务和审计记录。惩罚难度进度从房间级共享改为房间内按玩家个人计数，系列任务进度同步改为按玩家独立——你的进度只受你自己被罚次数影响。性别与阵营数据库化，后台可直接管理。系列任务未覆盖阵营时改为从随机池临时替补，避免卡关。投稿证明图缓存改为 6 小时，共建图片缓存为 30 天并永久保留作为审计材料。前端帮助文档新增共建玩法与难度对照表，供创作者参考。

#### 共建投稿系统
- **玩家投稿渠道**：大厅「参与共建」可投稿性别（挂到已有阵营）、随机任务、系列任务。创作者权限自动按身份等级控制（新号不开放）。
- **投稿与审核**：投稿可保存为草稿、提交审批。管理员在「后台 → 共建审核」查看，可批准、驳回（带批注）、编辑原稿后直即发布（无需投稿者再确认）、覆盖点赞率、申请下架。已上线内容的修订明确显示「修订草稿」「修订审核中」「修订被驳回」等独立状态，复审期间旧版继续可用。
- **版本与审计**：每份投稿、每次修订、每个管理员编辑都保留完整版本历史；所有正式任务/系列统一记录投稿版本和发布时的事务快照。
...
- **性别投稿**：持久玩家可新增性别并指定归属阵营；管理员可在「后台 → 性别与阵营」查看、编辑、删除。删除性别后，引用它的玩家自动重置为列表第一项并写库。性别与阵营运行时权威在 SQLite（`gender_factions` / `gender_options`，schema v33），`genders.json` / `gender-factions.json` 仅作首次导入种子。
...
#### 协议与数据库
- 消息类型新增：`contribution:list` / `get` / `saveDraft` / `submit` / `withdraw` / `requestUnpublish`（玩家投稿 RPC）；`contribution:votePreview` / `vote`（评价相关）；`admin:action` 的 `contribution*` / `genders*` 系列（管理员操作）。
- HTTP 端点新增：`POST /api/contribution-image`（投稿封面上传）。
- Schema v33–v40：v33 新增 `gender_factions` / `gender_options` 表（性别与阵营库化）；v34 为 `punishment_series` 增加 `target_faction_ids`；v35 为 `punishment_tasks` 增加 `task_group_id`（投票粒度）；v36 增加 `created_at`（分析用）；v38 为 `players` 补齐 10 个字段（白给自救板与改名状态落库）；v39 删除 `punishment_tasks.name`；v40 补齐索引。
